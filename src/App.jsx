...
const handleDownloadExcel = async () => {
  ...
  const buildFuelMap = (dt0, dt1) => {
    const map = {};
    reports.forEach(r => {
      const t = new Date(r.ts).getTime();
      if (t >= dt0 && t <= dt1) {
        const s = SHIPS.find(x => r.ship && r.ship.includes(x));
        if (s) { if (!map[s]) map[s] = {ME:0, AESea:0, AEPort:0}; const f = Number(r.fuel||0); const ph = Number(r.phaseRunning?.hours||0); map[s].ME += f*ph; }}
      }
    });
    for (const s in map) { const m = map[s]; m.ME = m.ME; m.AESea = m.AESea; m.AEPort = m.AEPort; }
    return map;
  };
  const fuelPrev = buildFuelMap(prevMonthStart.getTime(), prevMonthEnd.getTime());
  const fuelCur = buildFuelMap(monthStart.getTime(), monthEnd.getTime());
  const FuelPrevByShip = {};
  const FuelByShip = {};
  SHIPS.forEach(s => {
    FuelPrevByShip[s] = fuelPrev[s] || {ME:0, AESea:0, AEPort:0};
    FuelByShip[s] = fuelCur[s] || {ME:0, AESea:0, AEPort:0};
  });
          ...
          <tbody>
            {SHIPS.map((ship, idx) => (
              <tr key={ship}>
                ...
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{(() => { const v=SailingDaysByShip[ship]; return v !== null ? v.toFixed(2) : "—"; })()}</td>
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{(() => { const v=AnchorageDaysByShip[ship]; return v !== undefined ? v.toFixed(2) : "—"; })()}</td>
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{(() => { const v=AtPortDaysByShip[ship]; return v !== null ? v.toFixed(2) : "—"; })()}</td>
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{(() => { const v=DowntimeDaysByShip[ship]; return v !== undefined ? v.toFixed(2) : "—"; })()}</td>
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{daysInSelectedMonth}</td>
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{(() => { return (TotalDistanceByShip[ship] || 0).toFixed(1); })()}</td>
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{(() => { return (FuelPrevByShip[ship]?.ME||0).toFixed(1); })()}</td>
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{(() => { return (FuelPrevByShip[ship]?.AESea||0).toFixed(1); })()}</td>
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{(() => { return (FuelPrevByShip[ship]?.AEPort||0).toFixed(1); })()}</td>
                <td style={{ ...ss.td(idx%2), border:"1px solid rgba(45,120,185,0.28)", textAlign:"center" }}>{(() => { return (FuelByShip[ship]?.ME||0).toFixed(1); })()}</td>
